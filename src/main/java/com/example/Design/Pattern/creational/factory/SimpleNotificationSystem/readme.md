...
* Implement the Notification class as a common parent class for all notifications. Include attributes and methods that are common to all notifications.

* Implement the NotificationFactory class that implements the Simple Factory pattern. Add a method to create notifications based on NotificationType and other parameters.

* Run the provided test cases in the NotificationTest class to verify the correctness of your implementation. The tests will check if all notifications have a common parent class and if the factory class can correctly create notifications based on the notification type, recipient, and message.